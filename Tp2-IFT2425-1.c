...
// author  : Qiao Wang, qiao.wang@umontreal.ca ; Larissa Avononmadegbe, larissa.avononmadegbe@umontreal.ca 
// date    : 2020.03.11
// version : 1.0
// language: C++
// note    :
...
//Equation 5 Vecteur Moyen
float vecteurMoyen(float** v, int i, int j){
...
*/
 ...
 for(k = 0; k < NBITER-1; k++){
    ...
    for(i=1; i< length-1 ;i++) {
        ...
        VxM[i][j] = vecteurMoyen(OptFl_Vx[k],i,j);
        VyM[i][j] = vecteurMoyen(OptFl_Vy[k],i,j);

        float temp = ( Ix[i][j] * VxM[i][j] + Iy[i][j] * VyM[i][j] + It[i][j])
            /(alpha*alpha + pow(Ix[i][j],2) + pow(Iy[i][j],2));


        OptFl_Vx[k+1][i][j]= VxM[i][j] - Ix[i][j]*temp;
        OptFl_Vy[k+1][i][j]= VyM[i][j] - Iy[i][j]*temp;
      }
    }
 }
 ...
 //Sauvegarde
 printf("\n");
 SaveImagePpm((char*)"Optical_Flot_WithImg",Img2,SeqImgOptFlot[NBITER-1],length,width);
 SaveImagePgm((char*)"Optical_Flot",SeqImgOptFlot[NBITER-1],length,width);
