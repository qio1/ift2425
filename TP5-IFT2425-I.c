...
//Fonction 1 sans integral
static inline float fct1(float x){
	return 4 * sqrt( 1 - x * x );
}

//Somme par paires des elements d'un tableau
float sum(float* tab, int debut, int fin){
	if (fin - debut == 1)
		return tab[debut];

	int mi = (debut+fin)/2;
	return sum(tab,debut,mi) + sum(tab, mi,fin);
}

//Somme compensee de Kahan
float sumkahan(float* tab, const int nbInt){
	float temp = 0;
	float s = 0;
	float e = 0;
	float y = 0;
...
//--------------------------------------------------------------------------------
 ...
 result = 0;
 float h = 1 / ((float)NbInt+1); //N = 5 000 001
 float x,y;
 
 //I-1 estimation par methode trapeze
 for(int i = 0; i <= NbInt; i++){
	  x = i/((float)NbInt+1);
    y = (i+1)/((float)NbInt+1);

	  VctPts[i] = h/2.0*(fct1(x)+fct1(y)) ; // h/2(f_i-1 + f_i)
    
    result += VctPts[i];
    
 }
 printf("[1>Given_Order:] Pi=%.10lf Er=%.10lf LogEr=%.2lf\n",
    result, PI - result, log10(PI - result));
 

 //I-2-a estimation par la somme par paires
 result = sum(VctPts , 0 , NbInt + 1.0);

 printf("[2>PairwiseSum:] Pi=%.10lf Er=%.10lf LogEr=%.2lf\n",
    result, PI - result, log10(fabs(PI - result)));

 //I-2-b estimation par la somme compensee de Kahan
 result = sumkahan(VctPts,NbInt);

 printf("[3>KahanSummat:] Pi=%.10lf Er=%.10lf LogEr=%.2lf\n",
    result,fabs( PI - result), log10(fabs(PI - result)));
