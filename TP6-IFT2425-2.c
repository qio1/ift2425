...
// author  : Qiao Wang, Larissa Avononmadegde
// date    : 
// version : 1.0
// language: C++
// note    :
...
/************************************************************************/
/* FABRIQUE_WINDOW()							*/
/* Cette fonction crée une fenetre X et l'affiche ?l'écran.	        */
/************************************************************************/
...
/****************************************************************************/
/* CREE_XIMAGE()							    */
/* Crée une XImage ?partir d'un tableau de float                          */
...
/****************************************************************************/
/* CREE_XIMAGECOUL()							    */
/* Crée une XImage ?partir d'un tableau 3 d de float                       */
/* L'image peut subir un zoom.						    */
/****************************************************************************/
...
inline double dst(double x,double y,double x0,double y0){
  return fabs(x-x0)+fabs(y-y0);
}
//si la norme L1 < dist pour un certain aimant, retourne le numero d'aimant
//sinon retourne 0;
int aimant_proche(double x,double y, double dist){
  if(dst(x,y,X_1,Y_1) < dist ){return 1;}
  if(dst(x,y,X_2,Y_2) < dist ){return 2;}
  if(dst(x,y,X_3,Y_3) < dist ){return 3;}
  return 0;
}

int calcul_conv(double x0,double y0,double vx0, double vy0,int iter){
  const double dist_conv = 0.5;
  const int max_conv = 20;
  double x,y,vx,vy;
  ...
  for(int k = 1; k < iter; k++){
      ...
      int aimant = aimant_proche(xtmp,ytmp,dist_conv);

      if(aimant){
        iter_conv++;

          if(iter_conv >= max_conv){
            return k;
          }
...
int main (int argc, char **argv)
{
  ...
  //par une image couleur donn?par l'équation d'en bas... et non pas par 
  ...
  //Un exemple ou la matrice de points MatPict est remplie
  //par une image en niveaux de gris  donn?par l'équation d'en bas... et non pas par 
  ...
  for(i=0;i<HEIGHT;i++) {
    for(j=0;j<WIDTH;j++){
      ...
      double x = Xmin + j/(float)WIDTH * (Xmax-Xmin);
      double y = Ymax - i/(float)HEIGHT * (Ymax-Ymin);
      double vx = 0.0;
      double vy = 0.0;
      int nb_iter = calcul_conv(x,y,vx,vy,NB_INTERV);
      float gray = (float)nb_iter;
      MatPict[0][i][j] = MatPict[1][i][j] = MatPict[2][i][j] =gray;
